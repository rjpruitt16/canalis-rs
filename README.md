...
**Concurrent, self-pacing drain.** When an instance frees up, Canalis drains that tenant's *entire* backlog concurrently rather than popping one job per heartbeat, adjusting its own rate and concurrency live from the instance's reported tolerance (`X-Canalis-Max-Concurrent`/`X-Canalis-Rps`, falling back to Aquifer's `X-Aqueduct-*`/`X-Aquifer-*` names) — the same signal Aquifer's queue already listens for, propagated one layer out. Canalis starts conservative and lets the instance say how much faster it can go.
